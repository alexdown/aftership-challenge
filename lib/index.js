	...
	function Courier() {
		...
		this.dpduk = function(tracking_number, test_callback) {
			...
			var http_callback1 = function(response) {
				...
				response.on('end', function () {
					console.log('Courier | DPD UK | here the params of your search session ');
					//I now use JSON.parse(str).obj.searchSession to call the ws that return shipment tracking

					var searchSessionObj = JSON.parse(str).obj;
					var options2 = {
						host: 'www.dpd.co.uk',
						path: '/esgServer/shipping/delivery/?parcelCode=' + searchSessionObj.parcel[0].parcelCode,
						headers: {
					  		'Cookie': 'tracking=' + searchSessionObj.searchSession,
					  		'Accept': 'application/json, text/javascript'   //that's important. without this accept, it returns 406
						}
					};

					console.log('Courier | DPD UK | calling dpduk...' + console.dir(options2));
