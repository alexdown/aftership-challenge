 ...
 */
		...
		this.dpduk = function(tracking_number) {
			...
			var options = {
			  host: 'www.dpd.co.uk',
			  path: '/apps/tracking/?parcel=' + tracking_number + '&geoSession=f13860a0-c61a-11e3-9bef-03fadc5227b5&search',
			  headers: {'Cookie': 'tracking=9b5bc960-c61a-11e3-9283-0d89a76e3362'}
			};
			...
			callback = function(response) {
				...
				response.on('end', function () {
					console.log('-->dpduk answered (not showing str, it\'s an html page, so waay too long' + str);
					tracking_result.checkpoints = [];
					...
					jsdom.env(
						...
						function (errors, window) {
							if (errors !== null) {
								...
								var t = window.$("#history_panel .toggle-section table");
									//console.log("-->table content:", t[0].innerHTML);

								t.children().each(function() {
									console.log('--->checking item value, it is: ' + window.$(this).children()[0].innerHTML);
									if ('Date & Time' === window.$(this).children()[0].innerHTML ) {
										console.log('-->skipping first item...');
										return;
									}

									tracking_result.checkpoints.push({
										country_name: window.$(this).children()[1].innerHTML,
										message: window.$(this).children()[2].innerHTML,
										checkpoint_time: Date.parse( window.$(this).children()[0].innerHTML ).toISOString().substring(0, 19) //need to remove the timezone info. eh :/
									});
								});
...
//console.dir(Courier.usps('9102999999302024326992'));
//console.dir(Courier.hkpost('CP889331175HK'));
console.dir(Courier.dpduk('15502370264989N'));